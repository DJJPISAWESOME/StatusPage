...
export async function verifyRequests(browser){
 ...
 await board.clock.fastForward(5100);await portal.reload();await portal.getByRole('button',{name:'Pause',exact:true}).click();await board.clock.fastForward(5100);await board.waitForFunction(()=>window.testYT.state===2);
 await portal.getByRole('button',{name:'Resume',exact:true}).click();await board.clock.fastForward(5100);await board.waitForFunction(()=>window.testYT.state===1);
 await portal.getByRole('button',{name:'Restart current song',exact:true}).click();await board.clock.fastForward(5100);await board.waitForFunction(()=>window.testYT.position===0);
 await portal.locator('#request-volume').fill('0.25');await portal.locator('#request-volume').dispatchEvent('change');await board.clock.fastForward(5100);await board.waitForFunction(()=>window.testYT.volume===25);
 await portal.getByRole('button',{name:'Skip',exact:false}).click();await board.clock.fastForward(5100);await board.waitForFunction(()=>window.testYT.id==='aaaaaaaaaaa');await board.evaluate(()=>window.testYT.finish());await board.waitForFunction(()=>document.querySelector('#radio-state').textContent.includes('Waiting for requests'));assert.equal(current,null);
