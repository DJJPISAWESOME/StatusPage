...
try {
 ...
 assert.equal(await page.locator('#tv-content .tv-weather-tabs').evaluate(el=>getComputedStyle(el).opacity),'0');await page.keyboard.press('Shift');await page.getByRole('button',{name:'03 AI temperature outlook'}).click();assert.equal(await page.locator('#tv-content .tv-model-line').count(),2);assert.match(await page.locator('#tv-content .tv-now-marker text').textContent(),/NOW/);const markerPosition=await page.locator('#tv-content .tv-now-marker').getAttribute('transform');await page.clock.fastForward(2000);assert.notEqual(await page.locator('#tv-content .tv-now-marker').getAttribute('transform'),markerPosition);await page.screenshot({path:'artifacts/board-models.png',animations:'disabled'});
