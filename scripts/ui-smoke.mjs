...
try {
 ...
 await page.locator('#tv-skip').press('Enter');await page.waitForFunction(expected=>document.getElementById('tv-board').dataset.scene===expected,'weather');await page.locator('#tv-content .tv-current-metrics').waitFor();assert.match(await page.locator('#tv-content .tv-subtitle').innerText(),/Warren, RI/);assert.equal(boardWeatherRequests.some(params=>params.get('lat')==='41.73'&&params.get('lon')==='-71.28'),true);assert.equal(await page.locator('#tv-content .tv-current-metrics .tv-network-card').count(),6);await page.screenshot({path:'artifacts/board-weather.png',animations:'disabled'});
 ...
 await page.getByRole('button',{name:'03 AI temperature outlook'}).click();assert.equal(await page.locator('#tv-content .tv-model-line').count(),2);assert.match(await page.locator('#tv-content .tv-now-marker text').textContent(),/NOW/);const markerPosition=await page.locator('#tv-content .tv-now-marker').getAttribute('transform');await page.clock.fastForward(2000);assert.notEqual(await page.locator('#tv-content .tv-now-marker').getAttribute('transform'),markerPosition);await page.screenshot({path:'artifacts/board-models.png',animations:'disabled'});
