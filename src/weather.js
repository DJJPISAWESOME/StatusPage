...
export async function weather(point, env = {}, fetcher = fetch, cache = globalThis.caches?.default) {
  ...
  const currentTask = cachedJSON(`current-v2/${latitude}/${longitude}`, 900, async () => {
    const query = new URLSearchParams(base); query.set('current', 'temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,wind_speed_10m');
    query.set('hourly','precipitation_probability,precipitation,wind_gusts_10m');
