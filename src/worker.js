...
import { networkWatch, networkWatchPage, networkNames } from './network-watch.js';
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/asn-names') return json(await networkNames(url.searchParams.get('asns')));
      if (path === '/api/network-watch-page') return json(await networkWatchPage(url.searchParams,env));
