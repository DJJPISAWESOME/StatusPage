...
export function renderNetworkWatch(content,report,page){
  ...
  if(page===1){
    ...
    for(const network of report.networks){
      const card=node('article','tv-asn-card');card.append(node('span','tv-now-kicker',`AS${network.asn}`),node('h2','',network.name),node('strong','tv-asn-status',network.events.length?`${network.events.length} reported events in 7 days`:network.eventsAvailable?'No matching reports in 7 days':'Outage coverage unavailable'));
    ...
    if(page===2){
      ...
      for(const n of report.networks)paths.append(node('p','',`AS${n.asn} → ${n.neighboursAvailable?(n.downstream.length?n.downstream.map(a=>`AS${a}`).join(', '):'No downstream-side neighbours observed'):'Neighbour data unavailable'}`));
    ...
    if(section.limited)content.append(node('p','tv-watch-warning','Feed limit reached; this is a partial report. Open Radar for the full event history.'));
    const list=node('div','tv-watch-events');
    for(const event of events.slice(0,6)){
      const row=node('article','tv-watch-event');row.append(node('span','tv-watch-state',event.state),node('h2','',event.type),node('p','',event.description),node('small','tv-muted',`${date(event.start)} · ${event.asns.map(a=>`AS${a}`).join(', ')||event.countries.join(', ')}${event.via?.length?' · downstream of '+[...new Set(event.via.map(v=>`AS${v.parent}`))].join(', '):''}`));list.append(row);
    }
    ...
    content.append(list,node('p','tv-muted',`${events.length>6?`Showing the latest 6 of ${events.length} matching reports. `:''}Reports include ended events. Routing anomalies are detections, not proof of an ISP outage.`));
  }
